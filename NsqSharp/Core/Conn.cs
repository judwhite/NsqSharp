...
namespace NsqSharp.Core
{
  ...
  {
    private static readonly ILog Log = LogProvider.For<Conn>();
    ...
    public void WriteCommand(Command cmd)
    {
      ...
      {
        Log.ErrorException("IO Error", ex);
  ...
  public partial class Conn
  {
    ...
    private IdentifyResponse identify()
    {
      ...
      try
      {
        ...
        Log.DebugFormat("IDENTIFY response: {0}", respJson);
    ...
    private void auth(string secret)
    {
      ...
      Log.InfoFormat("Auth accepted. Identity: {0} {1} Permissions: {2}",
    ...
    private void readLoop()
    {
        ...
        while (doLoop)
        {
          ...
          switch (frameType)
          {
            ...
            case FrameType.Error:
              ...
              Log.ErrorFormat("protocol error - {0}", errMsg);
            ...
            default:
              ...
              Log.ErrorFormat("IO error - {0}", unknownFrameTypeEx.Message);
      ...
      finally
      {
        ...
        {
          Log.WarnFormat("delaying close, {0} outstanding messages", messagesInFlight);
        }
        _wg.Done();
      ...
      using (var select =
        Select
          ...
          .CaseReceive(_cmdChan, cmd =>
          {
            ...
            {
              Log.ErrorFormat("error sending command {0} - {1}", cmd, ex.Message);
              close();
          ...
          .CaseReceive(_msgResponseChan, resp =>
          {
            ...
            if (resp.success)
            {
              Log.DebugFormat("FIN {0}", resp.msg.Id);
              _delegate.OnMessageFinished(this, resp.msg);
              _delegate.OnResume(this);
            }
            else
            {
              Log.DebugFormat("REQ {0}", resp.msg.Id);
            ...
            {
              Log.ErrorException(string.Format("error sending command {0}", resp.cmd), ex);
              close();
            }
          })
    ...
    private void cleanup()
    {
      ...
      while (true)
      {
        ...
        if (msgsInFlight > 0)
        {
          ...
          {
            Log.WarnFormat("draining... waiting for {0} messages in flight", msgsInFlight);
        ...
        if (_readLoopRunning == 1)
        {
          if (DateTime.Now - lastWarning > TimeSpan.FromSeconds(1))
          {
            Log.Warn("draining... readLoop still running");
            lastWarning = DateTime.Now;
          }
          continue;
        }
